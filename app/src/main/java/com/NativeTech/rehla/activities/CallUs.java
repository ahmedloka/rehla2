...
public class CallUs extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        hud = KProgressHUD.create(CallUs.this)
        ...
        submit.setOnClickListener(v -> {
            ...
            {
                if (Validation.isConnected(CallUs.this)) {
                    if (!validateFields(Objects.requireNonNull(message.getText()).toString())) {
                    ...
                    buildDialog(CallUs.this).show().setCanceledOnTouchOutside(false);
                }
            ...
            else
            {
                if (Validation.isConnected(CallUs.this)) {
                    if (!validateFields(Objects.requireNonNull(message.getText()).toString())) {
                        message.setError("من فضلك ادخل الرسالة");
                    }
                    else {
                        addContactUs();
                    }
    ...
    {
        hud.show();
        AddContactUsRequestModel addContactUsRequestModel=new AddContactUsRequestModel();
        addContactUsRequestModel.setMessage(Objects.requireNonNull(message.getText()).toString());
        mSubscriptions.add(NetworkUtil.getRetrofitByToken(DataManager.getInstance().getCashedAccessToken().getAccess_token())
                .AddContactUs(addContactUsRequestModel)
